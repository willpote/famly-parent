...
function addRangeCommand(
  name: string,
  desc: string,
  fn: (opts: { child?: string; from?: string; to?: string; days?: number }) => Promise<void>,
): void {
  program
    .command(name)
    .description(desc)
  ...
  return async (opts: T) => {
    try {
      ...
      if (err instanceof FamlyError) {
        ...
        if (process.env.FAMLY_DEBUG && err.body !== undefined) payload.body = err.body;
